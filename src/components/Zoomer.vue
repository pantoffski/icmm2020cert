...
export default {
  ...
  methods: {
    ...
    limit() {
      // image must be bigger than container
      var minScale = Math.max(
          this.containerWidth / this.imgW,
          this.containerHeight / this.imgH
        ),
        scale = this.scale < minScale ? minScale : this.scale;
      this.scale = scale;
      var maxY = (scale - 1) / 2;
      var maxX = (scale - 1) / 2;
      ...
      this.$emit("size", {
        x: this.containerWidth * this.translateX,
        y: this.containerHeight * this.translateY,
        scale: scale
      });
    },
    calcTranslateLimit() {
